...
/**
 * destroy_pcb - frees memory associated with all spawned processes and
 *		 completely kills all the spawned processes
 *
...
{
	int i;
	if (pcb == NULL || pcb->status == MALLOC_ERR)
		return;
...
/**
 * init_process - initializes a process with default values until info is
 *		  gotten from the instance for such process
 *
 * @pcb: process control block
 *
...
/**
 * destroy_process - frees memory associated with a process and completely
 *		     kills such process
 *
 * @pcb: process control block
